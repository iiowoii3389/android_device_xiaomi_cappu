...
#define FW_PATCH_SETTLEMENT_DELAY_MS          500
#endif

/* Delay (ms) to wait after the post-firmware HCI_RESET command-complete is
 * received, before sending the second baud-rate update VSC. Some Broadcom
 * controllers (e.g. BCM4354) re-boot the firmware on HCI_RESET and need this
 * grace period before they will accept further VSCs. 0 = no extra delay. */
#ifndef HW_POST_RESET_DELAY_MS
#define HW_POST_RESET_DELAY_MS               500
#endif

/* Delay (ms) held between powering the controller off and powering it back on
 * during BT_VND_OP_POWER_CTRL. A short guard time is required so the BT_REG_ON
 * line is kept low long enough for the controller to fully discharge/reset,
 * otherwise a fast off->on toggle can leave the controller in a stale state
 * (e.g. still clocked at the previous high baud) and the initial HCI_RESET
 * after re-open will get no response. 0 = no extra delay. */
#ifndef BT_VND_PWR_TURN_ON_DELAY_MS
#define BT_VND_PWR_TURN_ON_DELAY_MS          300
#endif
