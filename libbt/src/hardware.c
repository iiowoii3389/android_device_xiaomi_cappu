...
extern uint32_t g_bt_fw_settlement_delay_ms;
extern uint32_t g_bt_post_reset_delay_ms;
extern uint32_t g_bt_pwr_turn_on_delay_ms;
extern uint32_t g_bt_pwr_settle_delay_ms;
void hw_cfg_wd_start(void);
void hw_cfg_wd_stop(void);

/******************************************************************************
**  Firmware-config watchdog (command retransmit + controller reset)
**
**  The BCM4356 controller occasionally drops the first HCI command sent right
**  after a reset / firmware-launch / power-on (ROM/boot timing) so that no
**  command-complete ever comes back.  This watchdog watches the in-flight
**  command: if no command-complete is seen within HW_CFG_CMD_TIMEOUT_MS it
**  silently re-sends the same command; if HW_CFG_MAX_RETRIES retransmits still
**  produce no reply it power-cycles the controller and restarts the bring-up,
**  so the upper stack never has to (which takes 15-20s).  After
**  HW_CFG_MAX_PWRCYCLE failed resets it gives up and reports failure.
******************************************************************************/
#include <pthread.h>
#include <time.h>
#ifndef HW_CFG_CMD_TIMEOUT_MS
#define HW_CFG_CMD_TIMEOUT_MS   400
#endif
#ifndef HW_CFG_MAX_RETRIES
#define HW_CFG_MAX_RETRIES      1
#endif
#ifndef HW_CFG_MAX_PWRCYCLE
#define HW_CFG_MAX_PWRCYCLE     3
#endif
static pthread_t      hw_cfg_wd_thread;
static pthread_mutex_t hw_cfg_wd_mutex  = PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t  hw_cfg_wd_cond   = PTHREAD_COND_INITIALIZER;
static volatile int   hw_cfg_wd_running = 0;   /* watchdog thread alive */
static volatile int   hw_cfg_wd_done    = 0;   /* fwcfg finished, thread exit */
static volatile int   hw_cfg_wd_armed   = 0;   /* a command is in-flight */
static struct timespec hw_cfg_wd_deadline;
static int            hw_cfg_retry_count = 0;
static int            hw_cfg_pwrcycle_count = 0;
static uint16_t       hw_cfg_last_opcode;
static uint16_t       hw_cfg_last_len;
static uint8_t        hw_cfg_last_cmd[HCI_CMD_MAX_LEN];

static void hw_cfg_wd_compute_deadline(void)
{
    clock_gettime(CLOCK_REALTIME, &hw_cfg_wd_deadline);
    hw_cfg_wd_deadline.tv_sec  += HW_CFG_CMD_TIMEOUT_MS / 1000;
    hw_cfg_wd_deadline.tv_nsec += (long)(HW_CFG_CMD_TIMEOUT_MS % 1000) * 1000000L;
    if (hw_cfg_wd_deadline.tv_nsec >= 1000000000L) {
        hw_cfg_wd_deadline.tv_sec++;
        hw_cfg_wd_deadline.tv_nsec -= 1000000000L;
    }
}

/* Arm the watchdog for a command that is about to be sent. */
static void hw_cfg_wd_arm(uint16_t opcode, const uint8_t *cmd, uint16_t len)
{
    if (!cmd || !len) return;
    if (len > HCI_CMD_MAX_LEN) len = HCI_CMD_MAX_LEN;
    pthread_mutex_lock(&hw_cfg_wd_mutex);
    hw_cfg_last_opcode = opcode;
    hw_cfg_last_len = len;
    memcpy(hw_cfg_last_cmd, cmd, len);
    hw_cfg_retry_count = 0;
    hw_cfg_wd_armed = 1;
    hw_cfg_wd_compute_deadline();
    pthread_cond_signal(&hw_cfg_wd_cond);
    pthread_mutex_unlock(&hw_cfg_wd_mutex);
}

/* Disarm: a command-complete arrived (progress). */
static void hw_cfg_wd_disarm(void)
{
    pthread_mutex_lock(&hw_cfg_wd_mutex);
    hw_cfg_wd_armed = 0;
    pthread_cond_signal(&hw_cfg_wd_cond);
...
static void *hw_cfg_wd_fn(void *arg)
{
    pthread_mutex_lock(&hw_cfg_wd_mutex);
    while (!hw_cfg_wd_done) {
        if (!hw_cfg_wd_armed) {
            pthread_cond_wait(&hw_cfg_wd_cond, &hw_cfg_wd_mutex);
            continue;
        }
        int rc = pthread_cond_timedwait(&hw_cfg_wd_cond, &hw_cfg_wd_mutex,
                                        &hw_cfg_wd_deadline);
        if (hw_cfg_wd_done || !hw_cfg_wd_armed)
            continue;
        if (rc != ETIMEDOUT)
            continue;

        /* No command-complete in time -> retransmit the pending command. */
        if (hw_cfg_retry_count >= HW_CFG_MAX_RETRIES) {
            /* Genuinely stuck (almost always the controller never finished
             * booting after power-on).  Retransmits of HCI_RESET did not help;
             * hand the failure back to the stack so it does a clean
             * close+reopen+power-cycle restart.  We fail in ~3.5s instead of
             * the ~8s startup timer, so overall recovery is much faster. */
            ALOGE("hw_cfg watchdog: no response to opcode 0x%04x after %d "
                  "retransmits, aborting fwcfg", hw_cfg_last_opcode,
                  hw_cfg_retry_count);
            hw_cfg_wd_armed = 0;
            pthread_mutex_unlock(&hw_cfg_wd_mutex);
            if (bt_vendor_cbacks)
        ...
        hw_cfg_retry_count++;
        uint16_t op = hw_cfg_last_opcode;
        uint16_t  len = hw_cfg_last_len;
        ALOGW("hw_cfg watchdog: no cback for opcode 0x%04x, retransmit "
              "(retry %d/%d)", op, hw_cfg_retry_count, HW_CFG_MAX_RETRIES);
        hw_cfg_wd_compute_deadline();

        /* Allocate + send outside the mutex to avoid nesting vendor callbacks. */
        pthread_mutex_unlock(&hw_cfg_wd_mutex);
        if (bt_vendor_cbacks) {
            HC_BT_HDR *p = (HC_BT_HDR *)bt_vendor_cbacks->alloc(
                BT_HC_HDR_SIZE + HCI_CMD_MAX_LEN);
            if (p) {
                p->event = MSG_STACK_TO_HC_HCI_CMD;
                p->offset = 0;
                p->layer_specific = 0;
                p->len = len;
                memcpy((uint8_t *)(p + 1), hw_cfg_last_cmd, len);
                bt_vendor_cbacks->xmit_cb(op, p, hw_config_cback);
            }
        }
        pthread_mutex_lock(&hw_cfg_wd_mutex);
        /* loop: keep waiting for the retransmit's command-complete */
    }
    hw_cfg_wd_armed = 0;
    hw_cfg_wd_running = 0;
    pthread_mutex_unlock(&hw_cfg_wd_mutex);
...
/* Start the watchdog thread (once per fwcfg).
 *
 * With the kernel-side BT_DEV_WAKE edge/lead-time fix in place the controller
 * no longer drops commands because it fell asleep, so responses are fast and a
 * retransmit can no longer race with a late reply (the original double-cback
 * hazard).  The watchdog now only fires for genuinely missing replies (almost
 * always the very first HCI_RESET, when the controller was not yet booted),
 * and re-sending HCI_RESET (idempotent) once the controller has finished
 * booting recovers transparently instead of stalling for the stack's
 * multi-second startup timer.  Hard-stuck cases are still handed back to the
 * stack via fwcfg_cb(FAIL) after a bounded number of retransmits. */
void hw_cfg_wd_start(void)
{
    pthread_mutex_lock(&hw_cfg_wd_mutex);
    hw_cfg_wd_done = 0;
    hw_cfg_wd_armed = 0;
    hw_cfg_retry_count = 0;
    hw_cfg_pwrcycle_count = 0;
    if (!hw_cfg_wd_running) {
        hw_cfg_wd_running = 1;
        if (pthread_create(&hw_cfg_wd_thread, NULL, hw_cfg_wd_fn, NULL) != 0)
            hw_cfg_wd_running = 0;
    }
    pthread_mutex_unlock(&hw_cfg_wd_mutex);
}

/* Stop the watchdog thread (fwcfg finished / aborted). */
void hw_cfg_wd_stop(void)
{
    pthread_mutex_lock(&hw_cfg_wd_mutex);
    hw_cfg_wd_done = 1;
    ...
    pthread_mutex_unlock(&hw_cfg_wd_mutex);
    if (hw_cfg_wd_running)
        pthread_join(hw_cfg_wd_thread, NULL);
}
...
uint32_t look_up_fw_settlement_delay (void)
{
    ...
    if (g_bt_fw_settlement_delay_ms > 0)
    {
        ret_value = g_bt_fw_settlement_delay_ms;
    }
#if (VENDOR_LIB_RUNTIME_TUNING_ENABLED == TRUE)
...
void hw_config_cback(void *p_mem)
{
    ...
    /* A command-complete arrived: the in-flight command is no longer pending,
     * cancel any pending watchdog retransmit. */
    hw_cfg_wd_disarm();

    ALOGI("hw_config_cback: ENTER state=%d opcode=0x%04x status=%d", hw_cfg_cb.state, opcode, status);
        ...
        switch (hw_cfg_cb.state)
        {
            ...
            case HW_CFG_DL_FW_PRE_PATCH:
                ...
                *p = 0; /* parameter length */
                ...
                hw_cfg_wd_arm(HCI_READ_LOCAL_NAME, (uint8_t *)(p_buf + 1), p_buf->len);
                ...
                if ((status = hw_config_findpatch(p_name)) == TRUE)
                {
                    ...
                    else
                    {
                        ...
                        *p = 0; /* parameter length */
                        ...
                        hw_cfg_wd_arm(HCI_VSC_DOWNLOAD_MINIDRV, (uint8_t *)(p_buf + 1), p_buf->len);
            ...
            case HW_CFG_DL_FW_PATCH:
                ...
                *p = 0; /* parameter length */
                ...
                ALOGI("hw_cfg: post-fw: sending HCI_RESET after %d ms settlement", delay);
                hw_cfg_wd_arm(HCI_RESET, (uint8_t *)(p_buf + 1), p_buf->len);
                is_proceeding = bt_vendor_cbacks->xmit_cb(HCI_RESET, p_buf, hw_config_cback);
                ALOGI("hw_cfg: post-fw HCI_RESET xmit is_proceeding=%d", is_proceeding);
                break;

            case HW_CFG_START:
                if (g_bt_post_reset_delay_ms)
                {
                    /* On BCM4354 the reset triggered by HCI_RESET needs time to
                     * complete before the controller will accept the following
                     * baud-rate update VSC; otherwise the command is lost.
                     * This applies both to the initial (post power-on) reset
                     * and to the post-firmware reset. */
                    ALOGI("hw_cfg: HCI_RESET ACKed (f_set_baud_2=%d), waiting %u ms before baud update",
                          hw_cfg_cb.f_set_baud_2, g_bt_post_reset_delay_ms);
                    ms_delay(g_bt_post_reset_delay_ms);
                }
            ...
            case HW_CFG_SET_UART_CLOCK:
                ...
                *p++ = 0; /* use encoded form */
                ...
                hw_cfg_wd_arm(HCI_VSC_UPDATE_BAUDRATE, (uint8_t *)(p_buf + 1), p_buf->len);
                is_proceeding = bt_vendor_cbacks->xmit_cb(HCI_VSC_UPDATE_BAUDRATE, \
                                                    p_buf, hw_config_cback);
                ALOGI("hw_cfg: sent UPDATE_BAUDRATE(%d) state=%d is_proceeding=%d f_set_baud_2=%d",
                      UART_TARGET_BAUD_RATE, hw_cfg_cb.state, is_proceeding, hw_cfg_cb.f_set_baud_2);
                break;
            ...
            case HW_CFG_SET_BD_ADDR:
                ...
                hw_sco_config();
#else
                ALOGI("hw_cfg: FWCFG SUCCESS, calling fwcfg_cb");
                hw_cfg_wd_stop();
...
void hw_config_start(void)
{
    ...
    /* NOTE: the watchdog is started by the BT_VND_OP_FW_CFG entry point in
     * bt_vendor_brcm.c so that a power-cycle recovery performed by the
     * watchdog itself can re-enter hw_config_start() without resetting the
     * power-cycle counter (which would defeat the retry limit). */
    ...
    if (p_buf)
    {
        ...
        *p = 0; /* parameter length */
        ...
        ALOGI("hw_config_start: sending initial HCI_RESET (state=%d)", hw_cfg_cb.state);
        hw_cfg_wd_arm(HCI_RESET, (uint8_t *)(p_buf + 1), p_buf->len);
