...
void userial_vendor_set_baud(uint8_t userial_baud)
{
    ...
    ALOGI("userial_vendor_set_baud: userial_baud=%d tcio=%d fd=%d", userial_baud, tcio_baud, vnd_userial.fd);

    /* Drain the transmit buffer before changing the baud so any command bytes
     * already queued go out at the OLD baud (they were formatted for it). */
...
/*******************************************************************************
**
** Function        userial_vendor_flush
**
** Description     Flush any pending RX/TX data on the vendor serial port.
**                 Used after a controller power-cycle to discard line noise
**                 so the HCI H4 packet parser does not desync on garbage.
**
** Returns         None
**
*******************************************************************************/
void userial_vendor_flush(void)
{
    if (vnd_userial.fd == -1)
        return;
    tcflush(vnd_userial.fd, TCIOFLUSH);
